...
line, in the exact form:
  - <command text>: passed
  - <command text>: failed
  - <command text>: skipped, <nonblank reason>
Use the command text exactly as it appears in the selected profile's
validationCommands. Do not claim a command "passed" without recording it
here. An optional command may be skipped only with a nonblank reason (e.g.
matching its declared environmentNotes prerequisite). The orchestrator does
not run these commands; only report commands you actually ran yourself.
