...
describe('documentation consistency gate', () => {
  ...
  it.each([
    ...
    ['wrong context-sensitive count', 'docs/WORKFLOWS.md', (text: string) => replace(text, 'existing 11-stage matrix', '10 context-sensitive stages'), 'CONTEXT_STAGE_COUNT_MISMATCH'],
    ...
    ['missing current-version publication', 'README.md', (text: string) => replace(text, '@dailephd/my-dev-kit-orchestrator@1.4.1', '@dailephd/my-dev-kit-orchestrator@1.2.9'), 'V123_PUBLISHED_CLAIM_MISSING'],
    ...
    ['blocker manifest field drift', 'docs/documentation-preservation-manifest.json', (text: string) => replace(text, '"blockingIssueCodes", "supportingIssueCodes"]', '"blockingIssueCodes", "secondaryCodes"]'), 'MANIFEST_FACT_DRIFT'],
    ...
    ['stale extraction artifact count', 'docs/USAGE.md', (text: string) => replace(text, 'five pre-implementation analysis stages produce six gate files', 'five pre-implementation analysis stages produce five gate files'), 'EXTRACTION_GATE_FILE_COUNT_DRIFT'],
