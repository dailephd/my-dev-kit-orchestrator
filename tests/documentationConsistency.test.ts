...
describe('documentation consistency gate', () => {
  ...
  it.each([
    ...
    ['wrong profile count', 'docs/WORKFLOWS.md', (text: string) => replace(text, /supports three starter\s+profiles:/, 'supports two starter profiles:'), 'GREENFIELD_PROFILE_COUNT_MISMATCH'],
    ...
    ['false v1.2.2 publication status', 'CHANGELOG.md', (text: string) => `${text}\nv1.2.2 is published.\n`, 'V122_PUBLICATION_FALSE_CLAIM'],
    ['missing v1.2.2 unreleased status', 'README.md', (text: string) => replace(text, 'Repository HEAD also contains the implemented, unreleased `v1.2.2` context-', 'Repository HEAD also contains the current safeguards for the next patch; the context-'), 'V122_UNRELEASED_STATUS_MISSING'],
    ...
    ['blocker manifest field drift', 'docs/documentation-preservation-manifest.json', (text: string) => replace(text, '      "supportingIssueCodes"\n    ]', '      "secondaryCodes"\n    ]'), 'MANIFEST_FACT_DRIFT'],
    ...
    ['v1.2.2 changelog batch history', 'CHANGELOG.md', (text: string) => replace(text, '## Unreleased - v1.2.2 Context Readiness and Documentation Safeguards', '## Unreleased - v1.2.2 Context Readiness and Documentation Safeguards\n\nBatch 4 implementation history.'), 'DOC_CHANGELOG_BATCH_LOG_CONTAMINATION'],
