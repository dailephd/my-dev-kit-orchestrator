...
3. `implementation` (not present in `test` mode) requires a `implementation responsibility ID:` block with `production file:` and/or `production symbol:` for every strategy RSP inside the existing `ImplementationReport`. The mapping is an authored declaration that my-dev-kit later corroborates; it does not prove causality, and a declared file may be an existing owner rather than a changed file. Before the stage is ready to advance, the agent refreshes the post-change index and updates the implementation-role context request, capsule/audit, packet, and retrieval report.
