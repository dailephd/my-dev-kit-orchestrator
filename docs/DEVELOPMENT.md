...
- `src/promptGenerator.ts`: packet/context prompt integration, refresh-only
  rendering, and (`v1.2.3`, implemented and unpublished) final-report blocked
  rendering
...
- `src/runIntegrityGate.ts` (`v1.2.3`, implemented and unpublished): the sole
  canonical run-integrity evaluator; every readiness-sensitive command reads
  its result instead of recomputing readiness
- `src/judgeIntegrity.ts` (`v1.2.3`, implemented and unpublished): authored
...
(`v1.2.3`, implemented and unpublished) additionally require:
...
worktree path is part of the public contract. `v1.2.3` (implemented and
unpublished) additionally consumes the corrected `my-dev-kit` v1.10.4
producer contract's additive, condition-aware evidence when a capsule/audit
declares it; v1.10.4 is itself implemented but not yet published, and
`1.10.3` evidence remains fully compatible without it.
