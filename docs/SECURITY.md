...
## Run telemetry (unpublished v1.6.0 source)

Native run telemetry is written only under the workspace `telemetry/` directory. Run and invocation identifiers must be single safe path segments (traversal, separators, drive forms, and reserved device names are rejected), every path is containment-checked with real path semantics, symbolic links and junctions are refused at every component, records are created exclusively without replacing an existing file, completed records are immutable, and each record is bounded to 8 KiB. Readers treat telemetry as untrusted data, report bounded diagnostics, and never mutate it; diagnostics never include absolute paths, stack traces, or raw record content. Records hold only bounded operational facts: prompt bodies, source or artifact content, `mark` reasons, environment values, credentials, command output, error text, and target-application data are never stored. A telemetry failure never changes a command's output or exit code and never affects workflow decisions. This is data-minimization and integrity hardening, not a cryptographic guarantee: a local user with file access can still alter or delete telemetry.
